"""
Focused Blockchain Data Extractor
Extracts only essential data: block headers, transactions, and optional token transfers/smart contract calls
"""
...
class FocusedDataExtractor:
    """
    Focused data extractor for essential blockchain data only
    """
    
    def __init__(self, provider_url: str = None):
        """
        Initialize focused data extractor
    ...
    def extract_block_headers(self, block_number: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                'transaction_count': len(block['transactions']),  # Just count, don't extract full data
                'extra_data': block['extraData'].hex() if block['extraData'] else None,
                'logs_bloom': block['logsBloom'].hex() if 'logsBloom' in block else None
            }
    ...
    def extract_token_transfers(self, tx_hash: str) -> List[Dict[str, Any]]:
        """
        Extract token transfers from a transaction (optional)
        ...
        """
        try:
            # Get transaction receipt to check for token transfer events
            ...
            for log in receipt['logs']:
                if len(log['topics']) >= 3 and log['topics'][0].hex() == transfer_event_signature:
                    # This is likely an ERC-20 transfer
                    token_transfer = {
                        'tx_hash': tx_hash,
                        'token_address': log['address'],
                        'from_address': '0x' + log['topics'][1].hex()[-40:],  # Remove padding
                        'to_address': '0x' + log['topics'][2].hex()[-40:],    # Remove padding
                        'value': int(log['data'], 16),  # Token amount
    ...
    def extract_smart_contract_calls(self, tx_hash: str) -> Optional[Dict[str, Any]]:
        """
        Extract smart contract call information (optional)
        
        Args:
            tx_hash: Transaction hash to analyze
            
        Returns:
            Smart contract call data or None
        """
        try:
    ...
    def extract_block_data(self, block_number: int, include_token_transfers: bool = False, 
        ...
        if include_token_transfers:
            ...
            for tx in transactions:
                transfers = self.extract_token_transfers(tx['tx_hash'])
        ...
        if include_contract_calls:
            contract_calls = []
            for tx in transactions:
                call_data = self.extract_smart_contract_calls(tx['tx_hash'])
                if call_data:
...
if __name__ == "__main__":
    # Test the focused data extractor
